package nl.skope.android.widget;
...
public class RowLayout extends ViewGroup {
    ...
    protected void onLayout(boolean changed, int leftPosition, int topPosition, int rightPosition, int bottomPosition) {
        ...
        ArrayList<View> row = new ArrayList<View>();
        int i = 0;
        for (View child : getLayoutChildren()) {
            ...
            if (x + childWidth > widthOffset) {
            	/* CENTERING
            	// Get remaining space after wrapping
            	remainingSpace = (widthOffset - x)/2;
                // Center current row
                for (View rowChild: row) {
                	rowChild.layout(rowChild.getLeft() + remainingSpace, rowChild.getTop(), 
                			rowChild.getRight() + remainingSpace, rowChild.getBottom());
                }
                // Clear row
                row.clear();
                */
